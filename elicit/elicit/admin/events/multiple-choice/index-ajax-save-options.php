...
$existing = [];
$SQL = $ELICIT->prepare("SELECT id FROM multiple_choice_options WHERE poll_id = ?");
...
foreach ($options as $opt) {
    ...
    if ($option_id > 0 && isset($existing[$option_id])) {
        $SQL = $ELICIT->prepare("UPDATE `multiple_choice_options` SET `option` = ? WHERE `id` = ? AND `poll_id` = ?");
        $SQL->bind_param('sii', $option_text, $option_id, $poll_id);
        $SQL->execute();
        $SQL->close();
        $incoming_ids[] = $option_id;
    } else {
        $SQL = $ELICIT->prepare("INSERT INTO `multiple_choice_options` (`poll_id`, `option`) VALUES (?, ?)");
...
if (!empty($incoming_ids)) {
    ...
    $SQL = $ELICIT->prepare("DELETE FROM `multiple_choice_options` WHERE `poll_id` = ? AND id NOT IN ($placeholders)");
    $params = array_merge([$poll_id], $incoming_ids);
    $SQL->bind_param('i' . $types, ...$params);
    $SQL->execute();
    $SQL->close();
} else {
    $SQL = $ELICIT->prepare("DELETE FROM `multiple_choice_options` WHERE `poll_id` = ?");
    $SQL->bind_param('i', $poll_id);
    $SQL->execute();
    $SQL->close();
}
