...
$SQL = $ELICIT->prepare("DELETE FROM `rating_polls` WHERE `event_id` = ? AND `id` = ?");
$SQL->bind_param('ii', $event_id, $poll_id);

if ($SQL->execute() && $SQL->affected_rows > 0) {
    $stmt = $ELICIT->prepare("DELETE FROM `rating_responses` WHERE `poll_id` = ?");
    $stmt->bind_param('i', $poll_id);
    $stmt->execute();
    $stmt->close();

    $stmt = $ELICIT->prepare("DELETE FROM `sessions` WHERE `poll_type` = 'rating' AND `poll_id` = ? AND `event_id` = ?");
